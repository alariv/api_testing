...
const TableCell = memo(
			...
			<td key={cellKey} className={`market-${marketType}`}>
				<div
					className={`odds-widget market-${marketType} ${
						!isMilestone && marketData[currentLine]?.is_balanced
							? 'balanced'
							: ''
	...
	(prevProps, nextProps) => {
		...
		return (
			...
			JSON.stringify(prevProps.player.markets?.[prevProps.marketType]) ===
				JSON.stringify(nextProps.player.markets?.[nextProps.marketType])
		);
	}
);
